...
impl LoopEngine {
    ...
    // MIDI Stop (0xFC), same as clock-loss timeout: pause and retain Song
    // Position. A following Continue (0xFB) resumes from here. This is a
    // deliberate departure from the MIDI 1.0 spec convention (where Stop
    // implies reset to Song Position 0) — only Start (0xFA) resets position.
