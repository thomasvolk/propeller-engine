...
impl ClockEngine {
    pub fn new(output: Box<dyn ClockOutput>, initial_bpm: f64) -> ClockEngine {
...
fn run_clock_loop(
    ...
    fn do_stop(output: &mut dyn ClockOutput, phase_pulse: &mut u64, pulse_count: &AtomicU64) {
        *phase_pulse = 0;
        pulse_count.store(0, Ordering::Relaxed);
        ...
        match current_state {
            ClockState::Stopped => match receiver.recv() {
                Ok(ClockCommand::Start) => {
                    do_start(output.as_mut(), &mut anchor, &mut phase_pulse, &pulse_count);
            ...
            ClockState::Running => {
                ...
                match poll_result {
                    ...
                    Some(ClockCommand::Stop) => {
                        do_stop(output.as_mut(), &mut phase_pulse, &pulse_count);
...
mod tests {
    ...
    fn new_engine_is_stopped() {
        ...
        let engine = ClockEngine::new(Box::new(output), 120.0);
    ...
    fn start_transitions_to_running_and_sends_midi_start() {
        ...
        let engine = ClockEngine::new(Box::new(output), 6000.0);
        engine.start();
