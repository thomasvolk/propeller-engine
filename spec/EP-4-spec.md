...
**Blocking start** (F-22): `propeller start` must return only after the daemon is fully ready to process IPC commands — not merely after the socket file exists. The `daemonize` crate is not usable here because its `start()` makes the original process exit before the socket is bound. Instead, `cmd_start` uses a **self-spawn** approach: it spawns a child process running the hidden `daemon-run` subcommand via `std::process::Command`. The child is detached from the terminal by passing `process_group(0)` (which calls `setpgid(0,0)` in the child, creating a new process group) and redirecting all stdio to `/dev/null`. This replaces the `daemonize` crate dependency entirely.

The readiness probe in the polling loop connects to the socket, sends `{"command":"status"}\n`, and waits for a non-empty response line. A simple `UnixStream::connect()` probe is insufficient: the socket becomes connectable as soon as `UnixListener::bind()` is called inside `daemon::run()`, which occurs before `run_ipc_server` has entered its accept loop. Polling with a full IPC roundtrip guarantees that callers can immediately issue `project create` and `loop start` commands without a race. The poll retries every 50 ms and times out after 10 s.
...
| T-39 | Write test: after `propeller start` returns exit 0, a `{"command":"status"}` probe over the socket receives a valid JSON response without any additional sleep | test | F-22, AC-21 | T-38 |
| T-40 | Impl: replace `daemonize` crate usage in `cmd_start` with self-spawn via `std::process::Command` using hidden `daemon-run` subcommand; add `process_group(0)` and `Stdio::null()` on all three stdio streams; in the readiness poll loop, connect to the socket, write `{"command":"status"}\n`, and read a response line — return only when a non-empty line is received; retry every 50 ms up to 10 s timeout | impl | F-22 | T-38 |
