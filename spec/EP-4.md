...
| F-22 | The `propeller start` CLI command blocks until the daemon is fully ready to process IPC commands. The command sends a `{"command":"status"}` probe over the socket and waits for a valid JSON response; it returns with exit code 0 only after that response is received. If the daemon is not ready within 10 seconds, the command exits with a non-zero status and an error message. A simple socket-connectivity check is insufficient because the socket becomes connectable when `UnixListener::bind()` is called, which precedes `run_ipc_server` accepting connections. |
